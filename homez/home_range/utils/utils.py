...
def homez_distance_displacement(gdf):
    ...
    date_grouped = gdf.groupby("NST_date")
    ...
    for date, date_group in date_grouped:
        ...
        for collar_id, collar_id_group in collar_id_grouped:
            time_sorted_group = sort_time_values(collar_id_group, "NST Time")
            point_1 = (
                time_sorted_group.Latitude.iloc[0],
                time_sorted_group.Longitude.iloc[0],
            )
            point_2 = (
                time_sorted_group.Latitude.iloc[-1],
                time_sorted_group.Longitude.iloc[-1],
            )
            ...
            for i in range(1, len(time_sorted_group)):
                point_1 = (
                    time_sorted_group.Latitude.iloc[i - 1],
                    time_sorted_group.Longitude.iloc[i - 1],
                )
                point_2 = (
                    time_sorted_group.Latitude.iloc[i],
                    time_sorted_group.Longitude.iloc[i],
                )
